...
class ApiRouter:
    ...
    def _process_request(
        ...
        process_func: Callable[[falcon.Request], str],
    ):
        ...
        try:
            ...
            resp.media = (
                response_data if isinstance(response_data, dict) else json.loads(response_data)
            )
    ...
    def on_get_document_details(self, req: falcon.Request, resp: falcon.Response, document_id: str):
        """Get document details with transactions."""
    ...
    def on_get_insights(self, req: falcon.Request, resp: falcon.Response, document_id: str):
        """Get insights for specific document."""
    ...
    def _on_post_documents(self, req: falcon.Request) -> dict:
        return DocumentsResource.upload_and_process_document(
            req, self.document_dao, self.documents_file
        )

    def _on_get_documents(self, req: falcon.Request) -> dict:
        return DocumentsResource.get_all_documents(self.document_dao)

    def _on_get_document_details(self, req: falcon.Request, document_id: str) -> dict:
        return DocumentsResource.get_document_details(document_id, self.document_dao)

    def _on_get_insights(self, req: falcon.Request, document_id: str) -> dict:
        return InsightsResource.get_document_underwriting_insights(document_id, self.document_dao)

    def _on_get_health(self, req: falcon.Request) -> dict:
        return {"status": "healthy", "service": "Bank Processing API"}
